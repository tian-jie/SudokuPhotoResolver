...
class MainWindow(QTabWidget):
    ...
    def __init__(self):
        ...
        # 模型初始化
        self.model = tf.keras.models.load_model("data/models/cnn_fv.h5")  # todo 修改模型名称
        self.to_predict_name = "images/tim9.jpeg"  # todo 修改初始图片，这个图片要放在images目录下
        self.class_names = ['sudoku', 'others']  # todo 修改类名，这个数组在模型训练的开始会输出
        self.resize(900, 700)
        self.initUI()
    ...
    def initUI(self):
        ...
        self.result_percentage = QLabel("-")
        label_result.setFont(QFont('楷体', 16))
        self.result.setFont(QFont('楷体', 24))
        self.result_percentage.setFont(QFont('楷体', 24))
        right_layout.addStretch()
        right_layout.addWidget(label_result, 0, Qt.AlignCenter)
        right_layout.addStretch()
        right_layout.addWidget(self.result, 0, Qt.AlignCenter)
        right_layout.addWidget(self.result_percentage, 0, Qt.AlignCenter)
        right_layout.addStretch()
        right_layout.addStretch()
    ...
    def predict_img(self):
        ...
        self.result_percentage.setText('%.2f' % (outputs[0][result_index]*100))
